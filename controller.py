...
class Control():
    ...
    def hatirlatici_yazma(self):
        try:
            ...
            for i in self.sesBloklari[:-1]:
                dosya.writelines(i + " ")
            dosya.writelines("\n")
            self.seslendirme("Notlar kaydedildi")
            dosya.close()
        except:
            self.seslendirme("Bir hata oluştu")
    ...
    def komutBul(self):
        ...
        # Günlük sohbet
            ...
            self.seslendirme("Memleketim Türkiyem")
        elif "ADIN NE" in self.ses or "ISMIN NE" in self.ses:
            self.seslendirme("Adım Seira seninki nedir")
            self.sesKayit()
            self.seslendirme("Güzel bir ismin var")
        elif "KENDİNDEN BAHSET" in self.ses or "NASIL BIRISIN" in self.ses:
            self.seslendirme("Söylenecek fazla şey yok. Adım Seira sana hizmet için oluşturulmuş bir sesli asistanım.")
        ...
        elif "İNTERNET" in self.ses in self.ses:
        ...
        elif "SÖYLEDİKLERİMİ YAZ" in self.ses or "YAZI YAZ" in self.ses or "DEDİKLERIMİ YAZ" in self.ses:
        ...
        elif "TÜRK LİRASI" in self.ses or "DOLAR" in self.ses or "EURO" in self.ses:
            self.para()
        # İngilizce-Türkçe çeviri yapar
        elif "ÇEVİRİ" in self.ses:
            self.ceviriYap()
